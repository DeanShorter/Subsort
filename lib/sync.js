...
export async function syncYouTubeSubscriptions(accessToken, userId, existingChannels, onProgress = () => {}) {
  ...
  for (let i = 0; i < idsToFetch.length; i += 50) {
    ...
    if (data.items) {
      for (const item of data.items) {
        ...
        ch.topicUrls = topicUrls;
        ch.topicIds = item.topicDetails?.topicIds || [];
  ...
  // Phase 3: Save to Supabase (shared + per-user tables)
  onProgress('Saving to database…', `${channels.length} channels`, 96);

  // 3a: Upsert shared channel metadata
  const sharedRows = channels.map(ch => ({
    youtube_channel_id: ch.channelId,
    title: ch.name,
    custom_url: ch.customUrl || '',
    thumbnail_url: ch.thumbnail || '',
    thumbnail_high_url: ch.thumbnailHigh || '',
    ...
    topics: ch.topics || [],
    topic_urls: ch.topicUrls || [],
    topic_ids: ch.topicIds || [],
    keywords: ch.keywords || '',
    country: ch.country || '',
  }));

  const CHUNK = 100;
  for (let i = 0; i < sharedRows.length; i += CHUNK) {
    const { error } = await supabase.from('shared_channels').upsert(
      sharedRows.slice(i, i + CHUNK),
      { onConflict: 'youtube_channel_id' }
    );
    if (error) console.error('Shared channels save error:', error);
  }

  // 3b: Upsert per-user relationships
  const userRows = channels.map(ch => ({
    user_id: userId,
    youtube_channel_id: ch.channelId,
  ...
  for (let i = 0; i < userRows.length; i += CHUNK) {
    ...
    if (error) console.error('User channels save error:', error);
  }

  // 3c: Also upsert to legacy channels table (backward compat during migration)
  const legacyRows = channels.map(ch => ({
    user_id: userId,
  ...
  for (let i = 0; i < legacyRows.length; i += CHUNK) {
    const { error } = await supabase.from('channels').upsert(
      legacyRows.slice(i, i + CHUNK),
      { onConflict: 'user_id,channel_id' }
    );
    if (error) console.error('Legacy channels save error:', error);
  }
