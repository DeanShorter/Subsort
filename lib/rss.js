...
function parseRSSXml(xml) {
  ...
  for (const entry of entries) {
    ...
    // Classify with confidence: video, suspected_video, suspected_short, short
    ...
    let videoType;
    if (hasExplicitShort || isVertical) {
      videoType = 'short';             // definite short
    } else if (hashCount >= 3) {
      videoType = 'short';             // 3+ hashtags → definite short
    } else if (hashCount === 2) {
      videoType = 'suspected_short';   // 2 hashtags → likely short
    } else if (hashCount === 1) {
      videoType = 'suspected_video';   // 1 hashtag → could be either, lean video
    } else {
      videoType = 'video';             // no hashtags, not vertical → definite video
    }
