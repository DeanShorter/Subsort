...
function parseRSSXml(xml) {
  ...
  for (const entry of entries) {
    ...
    // Detect shorts: keywords, multiple hashtags, or vertical thumbnail
    const titleLower = (title || '').toLowerCase();
    const descLower = (description || '').toLowerCase();
    const combined = titleLower + ' ' + descLower;

    // Count consecutive hashtags — shorts often have #funny #comedy #viral patterns
    const hashtagCount = (titleLower.match(/#\w+/g) || []).length;
