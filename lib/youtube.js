 ...
 */
export async function fetchRecentVideos(channels, accessToken, { maxPerChannel = 5, daysBack = 14 } = {}) {
  ...
  for (let i = 0; i < eligibleChannels.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (ch) => {
        ...
        if (!uploadsResp.ok) {
          if (uploadsResp.status === 403) {
            const err = new Error('YouTube token expired');
            err.status = 403;
            throw err;
          }
          return [];
