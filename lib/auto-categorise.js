...
function assignSubcategory(category, textCorpus) {
  ...
  for (const [subcategory, keywordList] of Object.entries(keywords)) {
    ...
    if (score >= 1) scores[subcategory] = score;
  }
...
export async function persistAutoSort(supabase, user, assignments, dbCategories, onProgress) {
  ...
  for (const a of assignments) {
    ...
    if (!subId) continue;
    const { error: chErr } = await supabase.from('channels').update({ subcategory_id: subId }).eq('id', a.channel.id);
    if (chErr) console.error('[AutoSort] Failed to update channel subcategory:', a.channel.id, chErr.message);
    if (a.channel.channelId) {
      const { error: ucErr } = await supabase.from('user_channels').update({ subcategory_id: subId }).eq('youtube_channel_id', a.channel.channelId).eq('user_id', user.id);
      if (ucErr) console.error('[AutoSort] Failed to update user_channel subcategory:', a.channel.channelId, ucErr.message);
    }
