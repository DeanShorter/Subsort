...
export async function persistAutoSort(supabase, user, assignments, dbCategories, onProgress) {
  ...
  const { data: allCats, error: catFetchErr } = await supabase.from('categories').select('*');
  if (catFetchErr) console.error('[AutoSort] Failed to fetch categories:', catFetchErr);
  console.log('[AutoSort] Fetched categories:', (allCats || []).length, 'rows:', (allCats || []).map(c => c.name));
  const catLookup = {};
