...
export async function persistAutoSort(supabase, user, assignments, dbCategories, onProgress) {
  ...
  // Re-fetch subcategories for ID lookup
  ...
  const inserts = assignments
    .map(a => {
      ...
      return { channel_id: a.channel.id, category_id: catId, user_id: user.id };
    })
    .filter(Boolean);
  ...
  for (let i = 0; i < unique.length; i += 100) {
    ...
    if (error && error.code !== '23505') console.error('[AutoSort] Failed to insert channel_categories:', error);
