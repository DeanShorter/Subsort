...
export function autoCategoriseAll(channels, chIsUncategorised) {
  ...
  for (const ch of channels) {
    ...
    const category = categoriseChannel(ch) || 'Other';
    assignments.push({ channel: ch, category });
    assigned++;
  }

  return { assignments, assigned, skipped };
}
...
export async function persistAutoSort(supabase, user, assignments, dbCategories, onProgress) {
  ...
  // Insert channel_categories in batches
  ...
  for (let i = 0; i < inserts.length; i += 100) {
    await supabase.from('channel_categories').insert(inserts.slice(i, i + 100));
    assigned = Math.min(i + 100, inserts.length);
    onProgress?.(assigned, inserts.length);
  }
