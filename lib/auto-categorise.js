...
export async function persistAutoSort(supabase, user, assignments, dbCategories, onProgress) {
  ...
  if (newCats.length) {
    for (const name of newCats) {
      ...
      if (error && error.code !== '23505') { // ignore unique violation
        console.error('[AutoSort] Failed to create category:', name, error);
      }
    }
  }

  // Re-fetch categories to get IDs
  const { data: allCats, error: catErr } = await supabase.from('categories').select('*');
  if (catErr) console.error('[AutoSort] Failed to fetch categories:', catErr);
  const catLookup = {};
  (allCats || []).forEach(c => { catLookup[c.name] = c.id; });

  // Build channel_categories rows
  const inserts = assignments
    .map(a => {
      const catId = catLookup[a.category];
      if (!catId) console.warn('[AutoSort] No category ID for:', a.category, '— available:', Object.keys(catLookup));
      return catId ? { channel_id: a.channel.id, category_id: catId, user_id: user.id } : null;
    })
    .filter(Boolean);
  ...
  for (let i = 0; i < unique.length; i += 100) {
    ...
    const { error } = await supabase.from('channel_categories').upsert(batch, { onConflict: 'channel_id,category_id', ignoreDuplicates: true });
    if (error) console.error('[AutoSort] Failed to insert channel_categories batch:', error);
    assigned = Math.min(i + 100, unique.length);
    onProgress?.(assigned, unique.length);
  }

  console.log(`[AutoSort] Done: ${newCats.length} new categories, ${unique.length} assignments`);
  return { categoriesCreated: newCats.length, channelsAssigned: unique.length };
}
