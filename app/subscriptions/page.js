  ...
  return (
    <main className="home-main">
      <PageHeader
        ...
        right={<>
          <button className="ph-btn" onClick={cycleSort}>
            <svg viewBox="0 0 14 14"><path d="M2 4h10M4 7h6M6 10h2" /></svg>
            Sort: <span>{SORT_OPTIONS[sortIdx].label}</span>
          </button>
          <button className="ph-btn" onClick={() => window.__subsortScrollToCats?.()}>
            <svg viewBox="0 0 14 14"><path d="M2 4h10M4 7h6M6 10h2" /></svg>
            Filters
          </button>
          <div className="ph-view-toggles">
            {['grid', 'list', 'hybrid'].map(mode => (
              <button key={mode} className={`ph-view-btn${chanView === mode ? ' active' : ''}`} title={mode.charAt(0).toUpperCase() + mode.slice(1)} onClick={() => setChanView(mode)}>
          ...
          <button className={`ph-btn${bulkMode ? ' active' : ''}`} onClick={toggleBulkMode}>
            <svg viewBox="0 0 14 14"><rect x="1" y="3" width="12" height="2" rx=".5" /><rect x="1" y="7" width="8" height="2" rx=".5" /><rect x="1" y="11" width="10" height="2" rx=".5" /></svg>
          ...
          <div className="ph-search-wrap">
            ...
            <input className="ph-search" placeholder="Search channels…" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
        </>}
      />
