  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const { data: { session } } = await supabase.auth.getSession();
        if (session?.access_token) {
          const capRes = await fetch('/api/apply-cap', { method: 'POST', headers: { Authorization: `Bearer ${session.access_token}` } });
          const capData = await capRes.json();
          if (capData.limited) {
            console.log(`[Onboarding] Free tier cap applied: ${capData.active} active, ${capData.locked} locked`);
          }
        }
      } catch {}
        ...
        {step === 'welcome' && (
          <div className="ob-body">
            ...
            <button className="ob-btn-primary" onClick={() => {
              // If we have a token OR user is already authenticated, skip Connect and go straight to syncing
              if (accessToken || user) {
                setStep('syncing');
              } else {
                setStep('connect');
