...
export default function SplitView({ panels, onRemovePanel, onSwapPanels, scrollRef }) {
  ...
  useEffect(() => {
    ...
    if (!scrollEl || panels.length === 0) { setMinibarVisible(false); return; }
  ...
  return (
    <>
      <div className="split-wrap" ref={splitRef}>
        <div className="split-label">
          <div className="split-label-icon">
                  ...
                  <div className="sp-info">
                    ...
                    <div className="sp-now">
                      <div className="sp-now-bars">
                        <div className={`sp-now-bar ${i === 0 ? 'muted' : 'unmuted'}`} />
                        <div className={`sp-now-bar ${i === 0 ? 'muted' : 'unmuted'}`} />
                        <div className={`sp-now-bar ${i === 0 ? 'muted' : 'unmuted'}`} />
                      </div>
                    </div>
                    <div className="sp-info-controls">
                      {panels.length === 2 && (
                        <button className="sp-btn" title="Swap panels" onClick={onSwapPanels}>
                          <svg viewBox="0 0 14 14"><path d="M4 2l-3 3 3 3" /><path d="M10 6l3 3-3 3" /><path d="M1 5h10M13 9H3" /></svg>
                        </button>
                      )}
                      <button className="sp-btn close-btn" title="Close panel" onClick={() => onRemovePanel(i)}>
      ...
      {/* Sticky minibar */}
      <div className={`split-minibar${minibarVisible ? ' show' : ''}`}>
        ...
        <div className="minibar-back" onClick={scrollToSplit}>
